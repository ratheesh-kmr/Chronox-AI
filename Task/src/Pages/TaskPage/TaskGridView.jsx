...
export default function TaskGridView({ tasks, onEdit, onTaskUpdate }) {
  ...
  const renderTaskCard = (task) => {
    ...
    return (
      ...
      >
        {/* Action Buttons - Only show for non-employees */}
        {canManageTasks && (
        ...
        <div className="flex items-center text-xs text-gray-500 mb-1">
          ...
          Project: <span className="ml-1 text-gray-800">{task.projectName?.projectName || "None"}</span>
        </div>
  ...
  {isExpanded && (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="mt-4 text-xs sm:text-sm space-y-2"
    >
      <p>
        <CalendarDays className="inline mr-1" size={12} />
