    ...
    >
      ...
      <div className="text-xs text-gray-400 space-y-1 mt-3 border-t pt-2">
        ...
        <div className="flex justify-between items-center">
          ...
          <span className="truncate max-w-[150px]" title={task.duration || "24hrs"}>
            {task.duration || "N/A"}
          </span>
        </div>
        <div className="flex justify-between items-center">
...
function KanbanBoard({ tasks, onTaskUpdate }) {
  ...
  const getColumnByTaskId = (taskId) =>
    Object.keys(columnsData).find((col) =>
      ...
      <DragOverlay>
        {activeTask ? (
          <div className="bg-white p-3 shadow rounded-md w-[250px]">
            ...
            <p className="text-sm text-gray-500">{activeTask.description || "No description"}</p>
          </div>
        ) : null}
      </DragOverlay>
    </DndContext>
...
// Parent component with split
export default function TeamLeadSplitKanban({ tasks, onTaskUpdate }) {
  ...
  return (
    <div className="space-y-12">
      ...
      <section>
        ...
        <KanbanBoard tasks={myTasks} onTaskUpdate={onTaskUpdate} />
      </section>

      {/* Team Tasks Kanban */}
      <section>
        <h2 className="text-xl font-semibold text-gray-700 mb-4">Team Tasks</h2>
        <KanbanBoard tasks={teamTasks} onTaskUpdate={onTaskUpdate} />
      </section>
    </div>
  );
}
