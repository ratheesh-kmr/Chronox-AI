...
const enhancedApiService = {
  ...
  //   return [
  //     { date: 'Jan', completed: 12, inProgress: 8, pending: 5 },
  //     { date: 'Feb', completed: 15, inProgress: 10, pending: 3 },
  //     { date: 'Mar', completed: 18, inProgress: 6, pending: 8 },
  //     { date: 'Apr', completed: 22, inProgress: 12, pending: 4 },
  //     { date: 'May', completed: 25, inProgress: 9, pending: 6 }
  //   ];
  // },
  ...
  //   return [
  //     { id: 1, title: "Design System Update", status: "In Progress", priority: "High", assignee: "John Doe", dueDate: "2025-08-25" },
  //     { id: 2, title: "API Integration", status: "Pending", priority: "Medium", assignee: "Jane Smith", dueDate: "2025-08-28" },
  //     { id: 3, title: "Database Migration", status: "Completed", priority: "High", assignee: "Mike Johnson", dueDate: "2025-08-20" },
  //     { id: 4, title: "UI Testing", status: "In Progress", priority: "Low", assignee: "Sarah Wilson", dueDate: "2025-08-30" }
  //   ];
  // },
  ...
  async fetchProjects() {
    return [
      { _id: '1', projectName: 'E-commerce Platform' },
      { _id: '2', projectName: 'Mobile App Development' },
      { _id: '3', projectName: 'Data Analytics Dashboard' },
      { _id: '4', projectName: 'Customer Portal' }
    ];
  },

  // Project Analytics by ID
  // async fetchProjectAnalyticsById(id) {
  //   const mockData = {
  //     '1': [
  //       { date: 'Week 1', completed: 8, inProgress: 12, pending: 5 },
  //       { date: 'Week 2', completed: 15, inProgress: 8, pending: 3 },
  //       { date: 'Week 3', completed: 22, inProgress: 6, pending: 2 },
  //       { date: 'Week 4', completed: 28, inProgress: 4, pending: 1 }
  //     ],
  //     '2': [
  //       { date: 'Week 1', completed: 5, inProgress: 10, pending: 8 },
  //       { date: 'Week 2', completed: 12, inProgress: 6, pending: 5 },
  //       { date: 'Week 3', completed: 18, inProgress: 4, pending: 3 },
  //       { date: 'Week 4', completed: 25, inProgress: 2, pending: 1 }
  //     ],
  //     '3': [
  //       { date: 'Week 1', completed: 3, inProgress: 8, pending: 12 },
  //       { date: 'Week 2', completed: 9, inProgress: 10, pending: 8 },
  //       { date: 'Week 3', completed: 16, inProgress: 7, pending: 4 },
  //       { date: 'Week 4', completed: 22, inProgress: 3, pending: 2 }
  //     ],
  //     '4': [
  //       { date: 'Week 1', completed: 10, inProgress: 15, pending: 7 },
  //       { date: 'Week 2', completed: 18, inProgress: 10, pending: 4 },
  //       { date: 'Week 3', completed: 25, inProgress: 6, pending: 2 },
  //       { date: 'Week 4', completed: 30, inProgress: 2, pending: 1 }
  //     ]
  //   };
  //   return mockData[id] || [];
  // },
  ...
  async fetchTeamMembers() {
    return [
      { id: 1, name: "John Doe", role: "Senior Developer", avatar: "/api/placeholder/32/32", status: "active", tasksCompleted: 23, efficiency: 92 },
      { id: 2, name: "Jane Smith", role: "UI/UX Designer", avatar: "/api/placeholder/32/32", status: "active", tasksCompleted: 18, efficiency: 88 },
      { id: 3, name: "Mike Johnson", role: "Project Manager", avatar: "/api/placeholder/32/32", status: "busy", tasksCompleted: 31, efficiency: 95 },
      { id: 4, name: "Sarah Wilson", role: "QA Engineer", avatar: "/api/placeholder/32/32", status: "offline", tasksCompleted: 15, efficiency: 85 },
      { id: 5, name: "Alex Chen", role: "DevOps Engineer", avatar: "/api/placeholder/32/32", status: "active", tasksCompleted: 20, efficiency: 90 }
    ];
  },
  ...
  async fetchOverdueTasks() {
    return [
      { id: 1, title: "API Integration", assignee: "John Doe", daysOverdue: 3, priority: "High" },
      { id: 2, title: "Database Migration", assignee: "Jane Smith", daysOverdue: 1, priority: "Medium" },
      { id: 3, title: "UI Testing", assignee: "Sarah Wilson", daysOverdue: 5, priority: "High" },
      { id: 4, title: "Performance Optimization", assignee: "Alex Chen", daysOverdue: 2, priority: "Low" }
    ];
  },
  ...
  async fetchTimeTracking() {
    return {
      ...
      weeklyData: [
        ...
        { day: 'Tue', hours: 9.2, billable: 8.1 },
        { day: 'Wed', hours: 7.8, billable: 6.9 },
        { day: 'Thu', hours: 8.9, billable: 7.8 },
        { day: 'Fri', hours: 8.1, billable: 7.0 }
      ]
    };
  },
  ...
  async exportReport(type, filters = {}) {
    ...
    const reportData = {
      ...
      tasks: await this.fetchProjectTasks(),
      team: await this.fetchTeamMembers(),
...
const StatCard = ({ title, value, change, changeType, icon: Icon, isHighlighted = false, onClick }) => {
  ...
  return (
    <div 
      className={`p-4 rounded-xl shadow-md cursor-pointer transition-all hover:shadow-lg ${
        isHighlighted ? "bg-purple-600 text-white" : "bg-white text-gray-800"
      }`}
      onClick={onClick}
    >
...
const Dashboard = () => {
  ...
  const API_CONFIG = [
    ...
    { name: 'Project Tasks', fn: enhancedApiService.fetchProjectTasks, setter: setProjectTasks },
    ...
    { name: 'Team Members', fn: enhancedApiService.fetchTeamMembers, setter: setTeamMembers },
    ...
    <main className="p-6 w-full bg-gradient-to-br from-purple-100 via-purple-200 to-pink-100 rounded-2xl min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          change={`${teamMembers?.filter(m => m.status === 'active')?.length} active`}
          icon={Users}
          changeType="positive"
        />
        <StatCard
              ...
              {teamMembers.slice(0, 5).map((member) => (
                <div key={member.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="font-medium text-sm text-gray-800">{member.name}</h4>
                      <p className="text-xs text-gray-500">{member.role}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="text-right">
                      <p className="text-xs font-medium text-gray-800">{member.tasksCompleted} tasks</p>
                      <p className="text-xs text-gray-500">{member.efficiency}% efficiency</p>
                    </div>
        ...
        <WidgetCard title="Overdue Tasks">
          {overdueTasks && overdueTasks.length > 0 ? (
              ...
              {overdueTasks.slice(0, 4).map((task) => (
                <div key={task.id} className="p-3 bg-red-50 border border-red-100 rounded-lg">
                  ...
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className="text-red-600 font-medium">{task.daysOverdue} days overdue</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="text-center py-8">
              <CheckCircle className="mx-auto w-8 h-8 text-green-400 mb-2" />
        ...
        {/* Risk Analysis */}
        <WidgetCard title="Risk Analysis">
          {riskAnalysis ? (
            <div className="space-y-4">
              <div className="grid grid-cols-1 gap-3 text-center">
                <div className="p-2 bg-red-50 rounded-lg">
                  <p className="text-lg font-bold text-red-600">{riskAnalysis.highRiskProjects}</p>
                  <p className="text-xs text-gray-600">High Risk Projects</p>
                </div>
                <div className="p-2 bg-orange-50 rounded-lg">
                  <p className="text-lg font-bold text-orange-600">{riskAnalysis.criticalTasks}</p>
                  <p className="text-xs text-gray-600">Critical Tasks</p>
                </div>
              </div>
              
              <div className="space-y-2">
                {riskAnalysis.risks && riskAnalysis.risks.slice(0, 3).map((risk, index) => (
            ...
            <div className="text-center py-8">
              <Target className="mx-auto w-8 h-8 text-gray-400 mb-2" />
