...
const enhancedApiService = {
  ...
  async fetchProjectAnalyticsById(id) {
    const response = await axiosInstance.get(`api/dashboard/analytics?projectId=${id}`);
    return response.data;
  },

  // People Management APIs
  async fetchTeamMembers() {
    const response = await axiosInstance.get("api/team/status");
    return response.data;
  },

  // Insights APIs
  async fetchOverdueTasks() {
    const response = await axiosInstance.get("api/task/overdue");
    return response.data;
  },

  async fetchTopPerformers() {
    const response = await axiosInstance.get("api/dashboard/top-performers");
    return response.data;
  },

  async fetchRiskAnalysis() {
    const response = await axiosInstance.get("api/dashboard/risk-analysis");
    return response.data;
  },

  // Time Tracking APIs
  async fetchTimeTracking() {
    const response = await axiosInstance.get("api/dashboard/time-tracking");
    return response.data;
  },

  // Export functionality
  async exportReport(type, filters = {}) {
    const response = await axiosInstance.post("api/dashboard/export", {
      type,
      filters,
    });
    return response.data;
