...
import { sendEmailOtp, sendMobileOtp, updateMobile } from "../../Services/services";
import { toast } from "react-toastify";

const ChangeMobileForm = ({ onClose }) => {
  ...
  const handleSendOtp = async () => {
    if (!mobile) return setErrors({ mobile: "Enter mobile number" });
  ...
  const handleVerify = async (e) => {
    ...
    try {
      setLoading(true);
      await updateMobile({ newMobile: mobile, otp });
  ...
  return (
    <div className="p-4 rounded-xl shadow-lg w-full max-w-md bg-white">
      <h2 className="text-lg font-bold mb-4 text-purple-700">Change Mobile Number</h2>
      <form onSubmit={handleVerify}>
        ...
        <div className="mb-3">
          <label className="flex items-center gap-2 text-sm mb-1 text-purple-600">
            <IconPhone size={16} /> New Mobile Number
          </label>
          <input
            type="tel"
            value={mobile}
